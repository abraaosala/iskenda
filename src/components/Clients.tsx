      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 space-y-4">
          ...
          <p id="clients-subtitle" className="text-base text-slate-500">
            Apoiamos ativamente empresas de variados setores na organização, crescimento sustentado
            e conformidade tributária em Angola.
          </p>
        </div>
        ...
        <div className="relative w-full overflow-hidden py-4 bg-slate-50 border border-slate-205 rounded-3xl mb-16">
          ...
          <div className="flex w-max animate-carousel-left space-x-8 items-center py-2">
            {doubleClients.map((client, index) => (
              ...
              >
                <div
                  className={`w-8 h-8 rounded-lg ${client.colorClass} flex items-center justify-center font-black text-sm uppercase shadow-inner`}
                >
                  {client.logoLetter}
                </div>
                <span className="text-xs sm:text-sm font-semibold text-slate-705 tracking-wide">
                  {client.name}
                </span>
              </div>
            ))}
        ...
        <div className="relative">
          ...
          >
            {clients.map((client) => (
              ...
              >
                <div
                  className={`w-10 h-10 rounded-xl ${client.colorClass} flex items-center justify-center font-extrabold text-white text-base mb-3 shadow-md group-hover:bg-white group-hover:text-brand-navy transition-all duration-305`}
                >
                  {client.logoLetter}
                </div>
        ...
        <div
          id="clients-subinfo"
          className="mt-12 text-center text-xs text-slate-400 flex items-center justify-center space-x-2"
        >
          <Landmark className="h-4 w-4 text-brand-orange" />
          <span>
            Contratos regulados de prestação de serviços com sigilo absoluto e conformidade
            regulatória.
          </span>
        </div>
      </div>
