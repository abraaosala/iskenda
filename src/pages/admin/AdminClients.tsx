  ...
  return (
    <>
      ...
      <div className="w-full">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 mt-0.5">Gerir logotipos de clientes</p>
            </div>
          </div>
        ...
        {loading ? (
          <LoadingOverlay text="A carregar clientes…" />
        ) : items.length === 0 ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
            {items.map((item) => (
              <div
                key={item.name}
                className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-sm transition-shadow text-center relative group"
              >
                {item.logo ? (
                  <img
                    src={item.logo}
                  ...
                  <div
                    className={`w-14 h-14 rounded-xl bg-gradient-to-br ${item.colorClass} flex items-center justify-center text-white font-bold text-lg mx-auto mb-2`}
                  >
                    {item.logoLetter}
                  </div>
                )}
...
function ClientModal({
  ...
  useEffect(() => {
    if (client)
      setForm({ name: client.name, logoLetter: client.logoLetter, colorClass: client.colorClass });
    setIsVisible(client?.isVisible !== false);
  }, [client]);
  useEffect(() => {
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      const payload: ClientPayload = {
        name: form.name.trim(),
        logo_letter: form.logoLetter.trim() || form.name.trim()[0].toUpperCase(),
        color_class: form.colorClass,
              ...
              <div className="flex flex-wrap gap-2">
                {colors.map((c) => (
                  <button
                    key={c}
                    type="button"
                    onClick={() => setForm({ ...form, colorClass: c })}
                    className={`w-8 h-8 rounded-lg bg-gradient-to-br ${c} border-2 ${form.colorClass === c ? "border-brand-navy" : "border-transparent"}`}
                  />
                ))}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Se desactivado, o cliente fica oculto para visitantes.
                </p>
              </div>
