  ...
  return (
    <>
      ...
      <div className="w-full">
        ...
        {loading ? (
          <LoadingOverlay text="A carregar equipa…" />
        ) : items.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-white rounded-xl border border-slate-200 p-5 hover:shadow-sm transition-shadow flex items-start justify-between"
              >
                <div className="flex items-center space-x-4 min-w-0">
                  {item.photo ? (
                    ...
                    <div
                      className={`w-12 h-12 rounded-xl bg-gradient-to-br ${item.gradient || "from-brand-navy to-brand-blue"} flex items-center justify-center text-white font-bold text-sm shrink-0`}
                    >
                      {item.initials}
                    </div>
                  )}
...
function TeamModal({
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    try {
      ...
      if (member) await updateTeamMember(member.id, payload);
            ...
            <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100">
              <div>
                ...
                <p className="text-xs text-slate-400 mt-0.5">
                  Se desactivado, o membro fica oculto para visitantes.
                </p>
              </div>
